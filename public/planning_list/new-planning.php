        ...
        <div class="row g-4 quadre">
            ...
            <div class="col-6">
                ...
                <select class="form-select" name="jobId" id="jobId">
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo "<option value='" . $row['id'] . "' data-job-name='" . $row['job_info'] . "'>" . $row['job_info'] . "</option>";
                    }
                    ?>
                </select>
            ...
            <div class="col-12 d-flex justify-content-between">
                ...
                <?php if (!isset($userId)) {
                    ...
                    echo '<a class="btn btn-secondary" href="'.APP_SERVER.'/planning-list/view/user/'.$userId.'" role="button">Come back to the Planning</a>
                    </div>';
                }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Llamar a updateJobName() y fetchJobHours() cuando la página se carga
    updateJobName();
    fetchJobHours(select.value);
    
    // Llamar a populateUserSelect() cuando la página se carga
    populateUserSelect(parseInt(<?php echo $userId; ?>)); // Pasar $userId como argumento
});
...
// Función para poblar el select de usuarios con una opción seleccionada por defecto
async function populateUserSelect(selectedUserId) {
  ...
  users.forEach(user => {
    ...
    // Seleccionar la opción correspondiente si es igual al selectedUserId
