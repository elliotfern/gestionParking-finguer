...
$currentStatus = intval($status); // Convertir el estado actual a entero
?>

<h1>Job list Sort by Status</h1>
...
<a href="<?php echo APP_SERVER;?>/job-list/" class="btn btn-secondary" role="button">All</a>
    <a href="<?php echo APP_SERVER;?>/job-list/status/1" class="btn <?php echo $currentStatus === 1 ? 'btn-warning' : 'btn-secondary'; ?>" role="button">Open</a>
    <a href="<?php echo APP_SERVER;?>/job-list/status/2" class="btn <?php echo $currentStatus === 2 ? 'btn-warning' : 'btn-secondary'; ?>" role="button">Quote</a>
    <a href="<?php echo APP_SERVER;?>/job-list/status/3" class="btn <?php echo $currentStatus === 3 ? 'btn-warning' : 'btn-secondary'; ?>" role="button">Quoted</a>
    <a href="<?php echo APP_SERVER;?>/job-list/status/4" class="btn <?php echo $currentStatus === 4 ? 'btn-warning' : 'btn-secondary'; ?>" role="button">Development</a>
    <a href="<?php echo APP_SERVER;?>/job-list/status/5" class="btn <?php echo $currentStatus === 5 ? 'btn-warning' : 'btn-secondary'; ?>" role="button">Invoice</a>
    <a href="<?php echo APP_SERVER;?>/job-list/status/6" class="btn <?php echo $currentStatus === 6 ? 'btn-warning' : 'btn-secondary'; ?>" role="button">Archive</a>
...
<div id="noResultsMessage" style="display: none; text-align: center; font-size: 18px; margin: 20px 0;">
...
function initializeDataTable() {
    loadJobList('<?php echo $status;?>');
}

function loadJobList(status) {
...
if (userId) {
  ...
  let urlAjax = "https://" + server + "/api/job/get/?type=jobs&status=" + status;
  let jobTable = $("#JobListTable");
    ...
    rowCallback: function(row, data, index) {
      if (data.status == 1) { // OPEN
        ...
        $(row).css('background-color', '#DD492E')
            .css('color', 'white')
            .addClass('status-' + data.status);
      } else if (data.status == 6) { // ARCHIVE
        $(row).css('background-color', '#2EA0DD')
            .css('color', 'black')
            .addClass('status-' + data.status);
      }
  },
    ...
    render: function (data, type, row, meta) {
        ...
        selectHTML += '<option name="status" value="6"' + (row.status == 6 ? ' selected' : '') + '>Archive</option>';
...

{
...

      {
        ...
        render: function (data, type, row, meta) {
          if (row.status == 6 ) {
            ...
            return '<button type="button" id="btnUpdateBook" class="btn btn-sm btn-danger">Archive</button>';
          }
        },
      },
    ],
 ...
 $('#JobListTable').on('change', 'select.statusSelect', function() {
    ...
    formulariActualizar2(userId, event, selectedStatus, id, "/api/job/put/?type=jobStatus"); 


    // AJAX PROCESS > PHP API : PER ACTUALIZAR FORMULARIS A LA BD
function formulariActualizar2(user, event, selected, id, urlAjax) {
