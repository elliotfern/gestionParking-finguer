<?php
$user = $params['user'];
$user = intval($user); // Convertir el estado actual a entero
?>

<h1>Job list</h1>
<h2>Sort by user: <span id="nameUser"></span></h2>
...
<div class="btn-group-toggle" data-toggle="buttons">
    <a href="<?php echo APP_SERVER;?>/job-list/user/<?php echo $user;?>" class="btn btn-warning" role="button">All Active Jobs</a>
...
if (userId) {
  ...
  let urlAjax = "https://" + server + "/api/job/get/?type=jobUser&user=" + status;
...
async function showUserName(userId) {
      const users = await fetchUsers();
      ...
      if (user) {
        ...
        document.getElementById('nameUser').innerText = 'User not found';
      }
    }

    showUserName('<?php echo $user;?>');
</script>